...
(function($, w) {
	...
	$.fn.validate = function() {
		var button = this,
			...
			fillError = 'Please fill out this field',
	...
	if ( $('.about-slider').length ) {
		...
		expandButton.click( function() {
			...
			if ( me.hasClass('collapse') ) {
				...
				setTimeout( function() { me.text('Expand'); }, 400);
			}
		});
	}
	...
	$('.tv-container').on('click', '.tv-item', function() {
		...
		videoText.text( me.find('.tvi-text').text() || "Не найдено описания" );
	...
	if ( document.getElementById('gc-map') ) {
		...
		var closeAddressList = function() {
			...
			filterCityTitle.text('All the cities');
		};
		...
		typesTable.switchTabs('td', true).on('click', 'td', function() {
			...
			if ( $(this).hasClass('active') ) {
				...
				filterTypeTitle.text('All types');
			}
		...
		placesTabs.on('click', 'li', function() {
			...
			if ( me.hasClass('active') ) {
				...
				filterCityList.empty().append('<li class="active"><span>All the cities</span></li>' + cityString);
				filterCityTitle.text('All the cities');
			...
			$.ajax({
				url: '/eng/about/client/json.php?country=' + me.data('country-id'),
				success: function(data, status, XHR) {
					...
					for ( var i in cityArray ) {
						cityString += '<li><span>' + cityArray[i] + '</span></li>';
					}
				...
				error: function(XHR, status, errorThrown) {
					...
					closeAddressList();
					placesList.empty().append('<p>Error while receiving information.</p>');
					g.placesData = [];
				},
				complete: function() {
	...
	dudes.click( function() {
		...
		$.ajax({
			url: '/eng/about/team/ajax.php?id=' + me.data('team-id'),
			success: function(data, status, XHR) {
				// console.log('AJAX ' + status + '. XHR status: ' + XHR.status + ' ' + XHR.statusText + '.');
